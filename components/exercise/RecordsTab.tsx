...
const RecordsTab: React.FC<RecordsTabProps> = ({ records, exerciseId }) => {
  const { t } = useI18n();
  const { profile, applyCalculated1RM, updateOneRepMax, undoAutoUpdate, allTimeBestSets } = useContext(AppContext);
  const { displayWeight, weightUnit } = useMeasureUnit();
            ...
            <div className="flex justify-between items-end mb-4">
                <div>
                    ...
                    <p className="text-3xl font-mono font-bold text-white">
                        {storedMax > 0 ? displayWeight(storedMax) : '-'} <span className="text-base text-text-secondary/50">{t(('workout_' + weightUnit) as TranslationKey)}</span>
                    </p>
                </div>
                {calculatedMax > 0 && (
