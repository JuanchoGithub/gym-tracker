  ...
  return (
    <svg width="100%" viewBox={`0 0 ${VIEWBOX_WIDTH} ${VIEWBOX_HEIGHT}`}>
      ...
      {yAxisLabels.map(label => (
        <g key={label.value}>
          <line x1={padding.left} y1={label.y} x2={VIEWBOX_WIDTH - padding.right} y2={label.y} stroke="#334155" strokeWidth="0.5" />
      ...
      {xAxisLabels.map((label, index) => (
        <text key={`${label.value}-${index}`} x={label.x} y={VIEWBOX_HEIGHT - 5} fill="#94a3b8" fontSize="10" textAnchor="middle">
          {label.value}
        </text>
      ))}
